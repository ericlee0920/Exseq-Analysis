...
results.to_csv("spatialDE_results.csv", index=False)
...
for i, g in enumerate(de_genes[:show]):
    ...
    # plt.axis('equal')
    plt.colorbar(ticks=[])
    plt.savefig(f"spatialDE_tissue_{g}.png")
    plt.clf()
...
C = 3
histology_results, patterns = SpatialDE.aeh.spatial_patterns(X, norm_expr, de_results, C=C, l=l, verbosity=1)
histology_results.to_csv("spatialDE_histology.csv", index=False)
patterns.to_csv("spatialDE_patterns.csv", index=False)
...
for i in range(3):
    ...
    plt.colorbar(ticks=[])
    plt.savefig(f"spatialDE_pattern_{i}.png")
    plt.clf()
